...
- **Source colour space** — `create --source-colourspace <rec709|xyz|…>` declares what the source carries. `rec709` (the default, byte-identical to before) leaves the compressor doing the DCI X'Y'Z' transform, `xyz` compresses untransformed. The wide-gamut and log spaces are refused rather than approximated: they need a real transform, which `colour --target xyz` or an HDR-to-DCI LUT does as its own pass
